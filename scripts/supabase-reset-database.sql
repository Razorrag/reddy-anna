...
-- ============================================
-- Complete database reset with fresh password hashes
...
DROP FUNCTION IF EXISTS update_balance_with_request CASCADE;
DROP FUNCTION IF EXISTS generate_referral_code CASCADE;
DROP FUNCTION IF EXISTS update_balance_atomic CASCADE;
DROP FUNCTION IF EXISTS cleanup_expired_tokens CASCADE;
DROP FUNCTION IF EXISTS update_updated_at_column CASCADE;
DROP FUNCTION IF EXISTS check_conditional_bonus CASCADE;
...
BEGIN
  WHILE code_exists LOOP
    ...
    SELECT EXISTS(SELECT 1 FROM users WHERE referral_code_generated = temp_code) INTO code_exists;
...
BEGIN
  ...
  UPDATE users SET balance = v_new_balance, updated_at = NOW() WHERE id = p_user_id;
...
BEGIN
  ...
  INSERT INTO request_audit (request_id, admin_id, action, old_status, new_status, notes)
  VALUES (p_request_id, p_admin_id, 'status_update', v_old_status, p_new_status, p_notes);
...
BEGIN
  ...
  IF p_new_status = 'approved' AND v_request.amount IS NOT NULL THEN
    IF v_request.request_type = 'deposit' THEN
      UPDATE users SET balance = balance + v_request.amount WHERE id = v_request.user_id;
    ELSIF v_request.request_type = 'withdrawal' THEN
      UPDATE users SET balance = balance - v_request.amount WHERE id = v_request.user_id;
    END IF;
    
    UPDATE admin_requests 
    SET balance_updated = true, balance_update_amount = v_request.amount
    WHERE id = p_request_id;
    
    INSERT INTO request_audit (request_id, admin_id, action, old_status, new_status, notes)
    VALUES (p_request_id, p_admin_id, 'balance_update', p_new_status, p_new_status, 
...
-- Bonus system function
CREATE OR REPLACE FUNCTION check_conditional_bonus(p_user_id VARCHAR(20))
RETURNS BOOLEAN AS $$
DECLARE
...
CREATE TRIGGER update_daily_stats_updated_at BEFORE UPDATE ON daily_game_statistics
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_monthly_stats_updated_at BEFORE UPDATE ON monthly_game_statistics
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_yearly_stats_updated_at BEFORE UPDATE ON yearly_game_statistics
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_stream_config_updated_at BEFORE UPDATE ON stream_config
    FOR EACH ROW EXECUTE FUNCTION update_stream_config_updated_at();

CREATE TRIGGER daily_stats_trigger AFTER INSERT ON player_bets
...
-- ============================================
-- STEP 9: INSERT DEFAULT GAME SETTINGS
-- ============================================
...
-- ============================================
-- STEP 10: INSERT DEFAULT STREAM SETTINGS
-- ============================================
...
-- ============================================
-- STEP 11: INSERT DEFAULT STREAM CONFIG
-- ============================================
...
-- ============================================
-- STEP 12: INSERT DEFAULT ADMIN DASHBOARD SETTINGS
-- ============================================
...
-- ============================================
-- STEP 13: CREATE ADMIN ACCOUNTS WITH FRESH HASHES
-- ============================================
-- 🔐 FRESH PASSWORD HASHES GENERATED: November 1, 2025
...
('admin', '$2b$12$kBXYropiQsR8cwFr386z/e9GuJSyGZbj1LctFVQ7tJcQU7HXfvrp6', 'admin'),
('rajugarikossu', '$2b$12$kBXYropiQsR8cwFr386z/e9GuJSyGZbj1LctFVQ7tJcQU7HXfvrp6', 'super_admin')
ON CONFLICT (username) DO UPDATE SET password_hash = EXCLUDED.password_hash, role = EXCLUDED.role;

-- ============================================
-- STEP 14: CREATE TEST USER ACCOUNTS WITH FRESH HASHES
-- ============================================
-- 🔐 FRESH PASSWORD HASHES GENERATED: November 1, 2025
...
('9876543210', '9876543210', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'Test Player 1 - VIP', 'player', 'active', 100000.00, 'TEST001'),
('9876543211', '9876543211', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'Test Player 2 - Premium', 'player', 'active', 50000.00, 'TEST002'),
('9876543212', '9876543212', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'Test Player 3 - Gold', 'player', 'active', 75000.00, 'TEST003'),
('9876543213', '9876543213', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'Test Player 4 - Silver', 'player', 'active', 25000.00, 'TEST004'),
('9876543214', '9876543214', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'Test Player 5 - Bronze', 'player', 'active', 10000.00, 'TEST005'),
('8686886632', '8686886632', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'Owner Test Account', 'player', 'active', 500000.00, 'OWNER001'),
('9999999999', '9999999999', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'Demo Player', 'player', 'active', 1000.00, 'DEMO001'),
('8888888888', '8888888888', '$2b$12$tbrgA//VDxYmIKQ37uAdE.ew7L7Wv6l5b65kivV0Xvir5HNdLx8cK', 'High Roller', 'player', 'active', 250000.00, 'HIGHROLL')
ON CONFLICT (id) DO UPDATE SET 
  password_hash = EXCLUDED.password_hash,
  balance = EXCLUDED.balance,
  full_name = EXCLUDED.full_name,
  status = EXCLUDED.status,
  referral_code_generated = EXCLUDED.referral_code_generated;

-- ============================================
-- STEP 15: INSERT TEST TRANSACTIONS
-- ============================================

INSERT INTO user_transactions (user_id, transaction_type, amount, balance_before, balance_after, status, description) VALUES
('9876543210', 'deposit', 100000.00, 0.00, 100000.00, 'completed', 'Initial test deposit'),
('9876543211', 'deposit', 50000.00, 0.00, 50000.00, 'completed', 'Initial test deposit'),
('9876543212', 'deposit', 75000.00, 0.00, 75000.00, 'completed', 'Initial test deposit'),
('9876543213', 'deposit', 25000.00, 0.00, 25000.00, 'completed', 'Initial test deposit'),
('9876543214', 'deposit', 10000.00, 0.00, 10000.00, 'completed', 'Initial test deposit'),
('8686886632', 'deposit', 500000.00, 0.00, 500000.00, 'completed', 'Owner account initial deposit'),
('9999999999', 'deposit', 1000.00, 0.00, 1000.00, 'completed', 'Demo account deposit'),
('8888888888', 'deposit', 250000.00, 0.00, 250000.00, 'completed', 'High roller account');
...
-- ============================================
-- STEP 17: DISABLE ROW LEVEL SECURITY (Development Only)
-- ============================================
...
SELECT COUNT(*) || ' admin accounts created' as admins FROM admin_credentials;
SELECT COUNT(*) || ' test users created' as users FROM users;
SELECT COUNT(*) || ' game settings configured' as settings FROM game_settings;
SELECT COUNT(*) || ' stream settings configured' as streams FROM stream_settings;
SELECT COUNT(*) || ' test transactions recorded' as transactions FROM user_transactions;
...
SELECT '👥 Test User Accounts:' as section;
SELECT id as phone, full_name, balance, status, referral_code_generated FROM users ORDER BY balance DESC;
...
--   Password: admin123
--
-- 👥 TEST USER ACCOUNTS (All passwords: Test@123):
--   Phone: 9876543210 - Test Player 1 VIP - Balance: ₹1,00,000
--   Phone: 9876543211 - Test Player 2 Premium - Balance: ₹50,000
--   Phone: 9876543212 - Test Player 3 Gold - Balance: ₹75,000
--   Phone: 9876543213 - Test Player 4 Silver - Balance: ₹25,000
--   Phone: 9876543214 - Test Player 5 Bronze - Balance: ₹10,000
--   Phone: 8686886632 - Owner Test Account - Balance: ₹5,00,000
--   Phone: 9999999999 - Demo Player - Balance: ₹1,000
--   Phone: 8888888888 - High Roller - Balance: ₹2,50,000
--
-- ✅ All hashes are FRESH and generated on: November 1, 2025
