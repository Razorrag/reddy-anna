...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const existingByMobile = await query('SELECT id FROM users WHERE mobile = ?', [mobile]);
      if (existingByMobile.length > 0) {
        return res.status(409).json({ message: 'Mobile already registered' });
      }

      if (email) {
        const existingByEmail = await query('SELECT id FROM users WHERE email = ?', [email]);
      ...
      await query(
        'INSERT INTO users (full_name, mobile, email, referral_code, password_hash, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const users = await query('SELECT id, full_name, mobile, email, password_hash FROM users WHERE mobile = ?', [mobile]);
      if (users.length === 0) {
...
router.get('/profile', async (req, res, next) => {
    try {
        ...
        const users = await query(
          'SELECT id, full_name, mobile, email, referral_code, date_of_birth, address, city, state, created_at FROM users WHERE id = ?',
          [userId]
        );
        if (users.length === 0) {
...
router.get('/dashboard', async (req, res, next) => {
    try {
        ...
        // Get user info
        const users = await query(
          'SELECT id, full_name, mobile, email, date_of_birth, address, city, state FROM users WHERE id = ?',
          [userId]
        );
        if (users.length === 0) {
            return res.status(404).json({ message: 'User not found' });
...
router.get('/admin/users', async (req, res, next) => {
    try {
        ...
        const blockedUsers = await query('SELECT user_id FROM blocked_users');
...
router.post('/admin/users', [
    ...
    try {
        ...
        const existingMobile = await query('SELECT id FROM users WHERE mobile = ?', [mobile]);
        ...
        // Insert new user
        await query(
          'INSERT INTO users (full_name, mobile, email, password_hash, date_of_birth, address, city, state, referral_code, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())',
...
router.put('/admin/users/:userId', [
    ...
    try {
        ...
        if (mobile) {
            const existingMobile = await query('SELECT id FROM users WHERE mobile = ? AND id != ?', [mobile, userId]);
            if (existingMobile.length > 0) {
...
router.put('/admin/users/:userId/block', [
    ...
    try {
        ...
        if (blocked) {
            ...
            const existingBlock = await query('SELECT id FROM blocked_users WHERE user_id = ?', [userId]);
            if (existingBlock.length === 0) {
              await query('INSERT INTO blocked_users (user_id, blocked_at) VALUES (?, NOW())', [userId]);
            }
        } else {
...
router.post('/admin/login', [
    ...
    try {
        ...
        const admins = await query('SELECT id, username, email, full_name, role, password_hash, is_active, last_login FROM admins WHERE username = ?', [username]);
