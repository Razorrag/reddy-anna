  ...
  async startCapture(): Promise<void> {
    ...
    try {
      ...
      if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        throw new Error('Screen sharing requires HTTPS connection');
      }
      ...
      try {
        ...
        if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
          ...
          throw new Error('Could not access screen. Another application may be using it.');
      ...
      if (!this.localStream || !this.localStream.getVideoTracks().length) {
        throw new Error('Failed to capture screen. No video track available.');
      }
