...
router.get('/settings/:key', async (req, res) => {
  try {
    ...
    const result = await query(
      'SELECT setting_key, setting_value, description FROM game_settings WHERE setting_key = ?',
      [key]
    );
...
router.put('/stream-settings', async (req, res) => {
  try {
    ...
    for (const setting of settings) {
      await query(
        'INSERT INTO stream_settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), updated_at = CURRENT_TIMESTAMP',
        [setting.key, setting.value]
      );
    }
