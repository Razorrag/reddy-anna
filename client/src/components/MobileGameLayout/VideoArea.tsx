...
const VideoArea: React.FC<VideoAreaProps> = React.memo(({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    if (isPausedState && wasJustPaused) {
      ...
      if (videoElement && streamConfig?.streamUrl?.includes('.m3u8')) {
        ...
        // ✅ CRITICAL FIX: Keep frozen frame visible until new video actually starts playing
        // DO NOT clear frozen frame here - let onPlaying event handle it
  ...
  // Determine if stream is live (truly independent of pause/active states)
  // Live when: not in loop mode and has URL (regardless of pause/active state)
