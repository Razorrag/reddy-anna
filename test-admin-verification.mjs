...
async function testAdminVerification() {
  try {
    ...
    // Get current admin from database using storage
    ...
    if (!currentMatch) {
      ...
      // Update the admin password hash directly in the database
      const { data: updatedAdmin, error: updateError } = await storage.supabase
