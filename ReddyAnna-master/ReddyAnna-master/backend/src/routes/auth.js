...
router.get('/profile', async (req, res, next) => {
    try {
        ...
        const users = await query('SELECT id, full_name, mobile, email, referral_code, date_of_birth, address, city, state, created_at FROM users WHERE id = ?', [userId]);
        if (users.length === 0) {
...
router.get('/dashboard', async (req, res, next) => {
    try {
        ...
        // Get user info
        const users = await query('SELECT full_name, mobile, email, date_of_birth, address, city, state FROM users WHERE id = ?', [userId]);
        if (users.length === 0) {
            return res.status(404).json({ message: 'User not found' });
        }
...
router.put('/admin/users/:userId/block', [
    ...
    try {
        ...
        if (blocked) {
            ...
            await query('INSERT IGNORE INTO blocked_users (user_id, blocked_at) VALUES (?, NOW())', [userId]);
        } else {
