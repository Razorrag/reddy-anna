...
1. **Admin Starts Stream:**
   - The UI (Layer 3) captures and crops the video.
   - It transfers the `videoTrack` to the Service Worker (Layer 2).
...
3. **Admin Returns or Page Finishes Reloading:**
   ...
   - The UI automatically captures and crops a **new** `videoTrack`.
...
async function initializeSFUConnection() {
  ...
  sfuConnection.onicecandidate = (event) => {
    if (event.candidate) {
      ...
      sendICECandidateToSFU(event.candidate);
    }
  };
...
async function establishSFUConnection() {
  // Get SFU offer from server
  const offer = await getSFUOffer(streamToken);
  ...
  await sendAnswerToSFU(streamToken, answer);
}
...
// Utility functions
async function getSFUOffer(streamToken) {
  const response = await fetch(`/api/sfu/offer/${streamToken}`, {
    method: 'POST',
    headers: {
...
async function sendAnswerToSFU(streamToken, answer) {
  await fetch(`/api/sfu/answer/${streamToken}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
async function sendICECandidateToSFU(candidate) {
  await fetch('/api/sfu/ice-candidate', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
export const ScreenShareCropper: React.FC<ScreenShareCropperProps> = ({ onCropComplete }) => {
  ...
  const setupVideoStream = async () => {
    try {
      // This would receive the stream from the service worker
      // In the new architecture, the actual crop happens in the service worker
      // This component just provides the UI for crop selection
    } catch (error) {
...
1. **Create service worker**
   ...
   - Add SFU connection and track management
...
1. **Update existing components**
   - Repurpose `StreamControlPanel.tsx`
   - Update `ScreenShareCropper.tsx`
...
### Client-Side:
1. **`public/stream-worker.js`** - Service worker for persistent streaming
...
- **Service Worker Security:** Validate all messages to/from service worker