...
try {
  // Check api-client.ts
  const apiClientContent = fs.readFileSync('client/src/lib/api-client.ts', 'utf8');
  ...
  // Check apiClient.ts
  const apiClientCamelCaseContent = fs.readFileSync('client/src/lib/apiClient.ts', 'utf8');
...
try {
  ...
  const serverRoutesContent = fs.readFileSync('server/routes.ts', 'utf8');
  ...
  const wsContextContent = fs.readFileSync('client/src/contexts/WebSocketContext.tsx', 'utf8');
...
try {
  ...
  const protectedAdminRouteContent = fs.readFileSync('client/src/components/ProtectedAdminRoute.tsx', 'utf8');
